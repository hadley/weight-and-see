...
ggplot(keywords, aes(week, weight = clicks)) +
  geom_histogram(binwidth = 1, fill="grey50") +
  scale_x_continuous("", breaks=c(1,14,27),
                     labels=c("Jan", "Apr", "Jul")) +
  ylab("Clicks")
...
@
  ...
  \caption{\label{webwords2}\em On the left is a histogram of weeks weighted by clicks and on the right the corresponding spinogram \hh{XXX not quite a spinogram yet - widths need to be adjusted - but it will make not much difference here. Is there a way to remove the spaces between the rectangles? XXX} with keyword of the company's name selected.}
\end{figure}
